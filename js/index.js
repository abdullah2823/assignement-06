...
const displayCategories = (treeDiv) => {
    treeDiv.forEach(cat => {
        treeCatergoris.innerHTML += `
            <div id="${cat.id}" class=" text-[16px] font-normal space-y-2">
               ...
               <ul>
                <li class="text-[16px] font-normal p-3 ">${cat.category_name}
                </li>
               </ul>
    ...
    const allPlants = (plantsId) => {
        ...
        fetch(`https://openapi.programming-hero.com/api/plants?category_id=${plantsId}`)
            .then(res => res.json())
            .then(data => {
    ...
    const allPlantsShow = (plants) => {
        plants.forEach(plants => {
            ...
            <div 
                        class=" bg-white shadow-md flex items-center justify-center p-3 py-10 max-w-[330px]  rounded-2xl">
                        <div  class="space-y-5">
                            <figure>
                                <img class=" rounded-lg  w-full  mx-auto rounded-2xl"
                                    src="${plants.image}"
                                    alt="">
                            </figure>
                            <div>
                                <h1 class="text-[16px]  font-semibold ">${plants.name} </h1>
                                <p class="text-gray-500">A fast-growing tropical tree that produces delicious, juicy
                                    mangoes during summer.
                                    Its dense green</p>
                            </div>
                            <div class="flex justify-between">
                                <button class="btn rounded-3xl bg-[#DCFCE7] hover:bg-green-700 hover:text-white">Fruit
                                    Tree
                                </button>
                                <p class="text-[16px] font-semibold px-2">$<span>500</span></p>
                            </div>
                            <button
